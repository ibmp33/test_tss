...
function gen_rsa_sha256() {
  ...
  openssl req -newkey rsa:4096 -nodes -sha256 -keyout client_rsa_sha256_key.pem \
    -subj "${SUBJECT}" -out server.csr

  openssl x509 -req -sha256 -extfile <(printf "subjectAltName=${ALT}") -days 3650 \
    -CA ca_cert.pem -CAkey ca_key.pem -CAcreateserial \
    -in server.csr -out client_rsa_sha256_cert.pem

  openssl pkcs12 -export -password pass:rocket \
    -in client_rsa_sha256_cert.pem -inkey client_rsa_sha256_key.pem -out client_rsa_sha256.p12
